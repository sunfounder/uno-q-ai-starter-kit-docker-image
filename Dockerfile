...
RUN apt-get update && apt-get install --fix-broken -y && apt-get install -y --no-install-recommends git build-essential && python -c "from urllib.request import urlretrieve; urlretrieve('https://raw.githubusercontent.com/sunfounder/sunfounder_tts/v1/docs/apt-requirements.txt', '/tmp/apt-reqs.txt')" \
